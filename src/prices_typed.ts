...
function createApp(database: Database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const date = parseDate(req.query.date as string, req.query.date as string);
    const cost = calculateCost(age, type, date, baseCost);
    res.json({ cost });
  });

  function parseDate(dateString2: string | undefined): Date | Temporal.PlainDate | undefined {
    if (dateString2 && typeof dateString2 === "string") {
      const temporalDate = Temporal.PlainDate.from(dateString2);
      return temporalDate;
    }
  }
  ...
  function isHoliday(date: Date | Temporal.PlainDate | undefined) {
    ...
    for (let row of holidays) {
      ...
      if (date && date instanceof Temporal.PlainDate && date.equals(temporalHoliday)) { return true };
    }
    return false;
  }

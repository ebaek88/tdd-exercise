...
function createApp(database: Database) {
  ...
  function parseDate(dateString: string | undefined, dateString2: string | undefined): Date | Temporal.PlainDate | undefined {
    if (dateString && typeof dateString2 === "string") {
  ...
  function isHoliday(date: Date | Temporal.PlainDate | undefined) {
    ...
    for (let row of holidays) {
      ...
      if (date && date instanceof Temporal.PlainDate && date.year === temporalHoliday.year && date.month === temporalHoliday.month && date.dayOfWeek === temporalHoliday.dayOfWeek)
        return true;
    }
    return false;
  }
